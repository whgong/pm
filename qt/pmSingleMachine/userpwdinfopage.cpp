...
void UserPwdInfoPage::setInitValue(){
    ...
    this->ui->txed_desc->setText(QString(this->crup->getObjName().c_str()));
}
...
{
    std::string *objnm, *usernm, *passwd, *desc;
    objnm = new std::string(ui->led_objnm->text().toStdString());
    usernm = new std::string(ui->led_usernm->text().toStdString());
    passwd = new std::string(ui->led_passwd->text().toStdString());
    desc = new std::string(ui->txed_desc->toPlainText().toStdString());
